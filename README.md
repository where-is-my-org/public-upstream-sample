...
- **跨平台支援**：提供適用於 Windows、Linux 和 macOS 的腳本版本，滿足不同作業系統的需求。
- **詳細的遷移生命週期規劃**：將遷移過程細分為六個階段，提供清晰的路徑指引與任務清單，確保每個階段的目標與成果明確。
- **Mannequin 認領流程**：提供專門的腳本與指導文件，協助組織有效管理遷移過程中產生的佔位使用者 (Mannequin)，確保最終環境的整潔與安全。
- **遷移前清理工具**：提供專門的腳本協助組織在遷移前進行儲存庫清理，包括辨識無活動的儲存庫、過期的 Pull Request 和 Issue，以及不必要的分支，確保遷移後環境的高效與整潔。
- **遷移後驗證與採用支持**：提供詳細的驗證步驟與使用者培訓建議，協助組織確保遷移後環境的穩定運行與使用者的順利過渡。

## 測試
- 手動加入 branch protection rule setup 的測試，確保腳本能正確辨識並處理受保護的分支。
- 啟用 branch protection
- 加入 branch protection bypass
- 設定 app 環境變數不新增 branch protection
